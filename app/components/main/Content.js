...
const Content = ({navigation}) => {
  ...
  const handleSearch = () => {
    if (
      `${selectedStartItem}`.trim() === '' ||
      `${selectedEndItem}`.trim() === '' ||
      !`${selectedDate}`.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/)
    ) {
      if (!visibleSnake) {
    ...
    <React.Fragment>
      <View style={styles.center}>
        <Card
          ...
          <Card.Content style={{marginLeft: 20, marginRight: 20}}>
            ...
            <Button
              icon={'magnify'}
              style={{marginTop: 12, backgroundColor: '#1f064f'}}
              mode="contained"
      ...
      <Portal>
        <Modal
          ...
          <AnimatedLottieView
            source={require('../../assets/searc_loader.json')}
            autoPlay={true}
            loop
          />
        </Modal>
